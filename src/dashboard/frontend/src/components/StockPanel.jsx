    ...
    <section className="panel">
      ...
      <div className="stock-grid">
        {stocks.map(s => {
          ...
          const chartData = Array.isArray(s.history) ? s.history.map((v, i) => ({ i, v: Number(v) })) : []
          return (
            <div key={s.symbol} className={`stock-card ${positive ? 'up' : 'down'}`}>
              ...
              <div className="price">${Number(s.price).toLocaleString()}</div>
              ...
              <ResponsiveContainer width="100%" height={50}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ background: '#1a1a2e', border: 'none', fontSize: 11 }} formatter={v => [`$${Number(v).toLocaleString()}`, '']} labelFormatter={() => ''} />
                </LineChart>
              </ResponsiveContainer>
              <div className="volume">Vol {(Number(s.volume) / 1e6).toFixed(1)}M</div>
            </div>
          )
        })}
      </div>
