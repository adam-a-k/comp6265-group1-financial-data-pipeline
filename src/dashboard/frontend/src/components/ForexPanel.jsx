    ...
    <section className="panel">
      ...
      <div className="forex-list">
        {pairs.map(p => {
          ...
          const chartData = Array.isArray(p.history) ? p.history.map((v, i) => ({ i, v: Number(v) })) : []
          return (
            <div key={p.pair} className="forex-row">
              ...
              <ResponsiveContainer width={120} height={36}>
                <AreaChart data={chartData}>
                  <defs>
                    <linearGradient id={`fg-${p.pair}`} x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="v" stroke={positive ? '#22c55e' : '#ef4444'} fill={`url(#fg-${p.pair})`} strokeWidth={1.5} dot={false} />
                  <Tooltip contentStyle={{ background: '#1a1a2e', border: 'none', fontSize: 10 }} formatter={v => [Number(v).toFixed(4), '']} labelFormatter={() => ''} />
                </AreaChart>
              </ResponsiveContainer>
              <div className="forex-rate">{Number(p.rate).toFixed(4)}</div>
            </div>
          )
        })}
      </div>
